...
class Marker(QtWidgets.QComboBox):
...
class LineEdit(QtWidgets.QLineEdit):
    ...
    def keyPressEvent(self, event, *args, **kwargs):
        if event.key() == QtCore.Qt.Key_Enter:
            ...
            QtWidgets.QLineEdit.keyPressEvent(self, event, *args, **kwargs)
...
class CurveModifyWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, item_id, mainwindow, *args, **kargs):
        ...
        self.properties_table.setCellWidget(0, 0, QtWidgets.QLabel('Color'))
        ...
        self.properties_table.setCellWidget(1, 0, QtWidgets.QLabel('Marker'))
        ...
        self.properties_table.setCellWidget(2, 0, QtWidgets.QLabel('Marker Size'))
...
class InfoWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, info_data, *args, **kwargs):
        ...
        for name, value in info_data.items():
            ...
            self.info_table.setCellWidget(index, 0, QtWidgets.QLabel(name))
            self.info_table.setCellWidget(index, 1, QtWidgets.QLabel(str(value)))
            index += 1

    #             self.info_table.setCellWidget(index, 0, QtWidgets.QLabel(des))
...
class ParamsWin(QtWidgets.QMainWindow):
    closed = pyqtSignal(bool)
    ...
    def __init__(self, params_data, changed_params_data, *args, **kwargs):
        ...
        self.choose_item_lineEdit = QtWidgets.QLineEdit(self)
        ...
        self.btn_changed_filter = QtWidgets.QPushButton('Changed')
        self.btn_changed_filter.clicked.connect(self.btn_changed_filter_clicked)
        w = QtWidgets.QWidget()
        self.vlayout = QtWidgets.QVBoxLayout(w)
        ...
        self.params_table.setEditTriggers(QtWidgets.QAbstractItemView.DoubleClicked |
        ...
        for name, value in self.params_data.items():
            if name in self.params_data_show:
                ...
                if name in self.changed_params_data:
                    ...
                    name_str = name
                    value_str = str(value)
                name_lbl = QtWidgets.QLabel(name_str)
                value_lbl = QtWidgets.QLabel(value_str)
                self.params_table.setCellWidget(index, 0, name_lbl)
                self.params_table.setCellWidget(index, 1, value_lbl)
                index += 1


class AnalysisGraphWin(QtWidgets.QMainWindow):
    ...
    def __init__(self, mainwindow, *args, **kwargs):
        ...
        self.graph_table = QtWidgets.QTableWidget(self)
            ...
            if graph_name == 'XY_Estimation':
                ...
                for name, label in data_name_dict.items():
                    if name == 'mode_slot':
                        ...
                        data.append(
                            (t, (self.mainwindow.log_data_list[data_index].data[name] == 1).astype(np.uint32), label))
                    else:
...
class HelpWin(QtWidgets.QMainWindow):
    closed = pyqtSignal(bool)
...
class QuadrotorWin(QtWidgets.QMainWindow):
    closed = pyqtSignal(bool)

    def __init__(self, *args, **kwargs):
        super(QuadrotorWin, self).__init__(*args, **kwargs)
        self.toolBar = self.addToolBar('showSetting')
        self.trace_show = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/trace.gif')),
                                            'show trace',
                                            self)
        self.trace_show.triggered.connect(self.callback_show_trace)
        self.trace_showed = False
        self.vector_show = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/rotor_vector.gif')),
        ...
        self.quadrotor_win_main_widget = QtWidgets.QWidget(self)
        self.quadrotor_win_main_layout = QtWidgets.QHBoxLayout()
