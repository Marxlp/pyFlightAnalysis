...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.basic_tool_group = QtWidgets.QActionGroup(self)
        ## load log file
        self.loadfile_action = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/open.gif')), 'Open log file', self)
        ...
        self.show_quadrotor_3d = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/quadrotor.gif')),
                                                   'show 3d viewer', self)
        ...
        ## show quadrotor info
        self.show_info = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/info.gif')), 'show log info', self)
        ...
        self.show_params = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/params.gif')), 'show params', self)
        ...
        self.show_analysis_graphs = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/analysis_graph.gif')),
        ...
        ## show help
        self.show_help = QtWidgets.QAction(QtGui.QIcon(get_source_name('icons/help.gif')),
                                           'show help', self)
        ...
        self.plot_data_frame = QtWidgets.QFrame(self)
        self.plot_data_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        #         self.plot_data_layout_H = QtWidgets.QHBoxLayout(self.plot_data_frame)
        self.plot_data_layout_V = QtWidgets.QVBoxLayout(self.plot_data_frame)
        ## Data Plotting [id, filesystem, ]
        self.data_plotting = OrderedDict()
        ### There exists a Default graph
        self.line_ID = 0
        lbl_ploting_data = QtWidgets.QLabel('Data Plotting')
        self.plotting_data_tableView = TableView(self.plot_data_frame)
        self.plotting_data_tableView.setEditTriggers(QtWidgets.QAbstractItemView.DoubleClicked |
        ...
        self.list_data_frame = QtWidgets.QFrame(self)
        self.list_data_frame.setMinimumWidth(400)
        self.list_data_frame.setMaximumWidth(600)
        self.list_data_frame.resize(400, 500)
        self.list_data_frame.setFrameShape(QtWidgets.QFrame.StyledPanel)
        self.list_data_layout = QtWidgets.QVBoxLayout(self.list_data_frame)
        ### line to search item
        self.choose_item_lineEdit = QtWidgets.QLineEdit(self.list_data_frame)
        ...
        self.item_list_treeWidget = QtWidgets.QTreeWidget(self.list_data_frame)
        ...
        self.animation_layout = QtWidgets.QVBoxLayout(self.graph_frame)
        ...
        ROI_action = QtWidgets.QAction('show/hide ROI graph', self.main_graph_t)
        ...
        ## time line
        self.time_line_frame = QtWidgets.QFrame(self)
        self.time_line_frame.setMaximumHeight(45)
        self.time_line_frame.setMinimumHeight(45)
        self.time_line_layout = QtWidgets.QHBoxLayout(self.time_line_frame)
        time_line_lbl = QtWidgets.QLabel('x')
        ...
        self.time_line_button_play = QtWidgets.QPushButton(self.time_line_frame)
        ...
        self.time_line_button_stop = QtWidgets.QPushButton(self.time_line_frame)
    ...
    def quat_to_euler(q0, q1, q2, q3):
        ...
        for i in range(len(q0)):
            roll = 180 / np.pi * np.arctan2(2.0 * (q0[i] * q1[i] + q2[i] * q3[i]),
                                            1.0 - 2.0 * (q1[i] ** 2 + q2[i] ** 2))
    ...
    def callback_open_log_file(self):
        ...
        filename = QtWidgets.QFileDialog.getOpenFileName(self, 'Open Log File', path, 'Log Files (*.ulg)')
    ...
    def animation_update(self):
        ...
        if self.ROI_showed:
            ...
            # emit data
            indexes = list(
                map(self.getIndex, [self.time_stamp_position, self.time_stamp_attitude, self.time_stamp_output],
                    [t, t, t]))
                    ...
                    if filtertext in key:
                        ...
                        for value in values_name:
                            if filtertext in value[0]:
                                values_satisfied.append(value)
                    if values_satisfied:
                        param_name = QtWidgets.QTreeWidgetItem(self.item_list_treeWidget, [key])
                        self.item_list_treeWidget.expandItem(param_name)
                        for data_name in values_satisfied:
                            self.item_list_treeWidget.expandItem(
                                QtWidgets.QTreeWidgetItem(param_name, [data_name[0], data_name[1], data_name[2]]))
    ...
    def callback_tree_double_clicked(self, item, col):
        ...
        # Label
        self.plotting_data_tableView.setCellWidget(row, 0, QtWidgets.QLabel(item_label))
    ...
    def keyPressEvent(self, event, *args, **kwargs):
        ...
        if event.key() == QtCore.Qt.Key_S:
            ...
            for curve in self.curve_highlighted:
                del (curve)
        return QtWidgets.QMainWindow.keyPressEvent(self, event, *args, **kwargs)
    ...
    def update_graph(self):
        ...
        for ind, (item_id, item) in enumerate(self.data_plotting.items()):
            ...
            self.plotting_data_tableView.setCellWidget(ind, 0, QtWidgets.QLabel(item[0]))
        ...
        if self.main_graph_t.legend.scene() is not None:
        ...
        for ind, (item_id, item) in enumerate(self.data_plotting.items()):
            ...
            if showed:
                curve = self.main_graph_t.plot(data[0], data[1], symbol=marker, pen=color, name=label,
    ...
    def draw_predefined_graph(self, name):
        ...
        if name == 'XY_Estimation':
            ...
            show_marker_action = QtWidgets.QAction('show/hide marker', graph_xy)
